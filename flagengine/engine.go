...
type featureContextWithSegmentName struct {
	featureContext *evalcontext.FeatureContext
	segmentName    string
}
...
func GetEvaluationResult(ec *evalcontext.EvaluationContext) evalcontext.EvaluationResult {
	const defaultPriority = 0.0

	segments := []evalcontext.SegmentResult{}
	flags := []evalcontext.FlagResult{}
	...
	for _, segmentContext := range ec.Segments {
		...
		segments = append(segments, evalcontext.SegmentResult{
	...
	if ec.Features != nil {
		for _, featureContext := range ec.Features {
			...
			if segmentFeatureCtx, exists := segmentFeatureContexts[featureContext.FeatureKey]; exists {
				...
				flags = append(flags, evalcontext.FlagResult{
					Enabled:    fc.Enabled,
	...
	return evalcontext.EvaluationResult{
		Context:  *ec,
		Flags:    flags,
		Segments: segments,
	}
...
func getFlagResultFromFeatureContext(featureContext *evalcontext.FeatureContext, identityKey *string) evalcontext.FlagResult {
	reason := "DEFAULT"
	...
	flagResult := evalcontext.FlagResult{
...
func isContextInSegment(ec *evalcontext.EvaluationContext, segmentContext *evalcontext.SegmentContext) bool {
...
func contextMatchesCondition(ec *evalcontext.EvaluationContext, segmentCondition *evalcontext.Condition, segmentKey string) bool {
	var contextValue segments.ContextValue
	if segmentCondition.Property != "" {
		contextValue = getContextValue(ec, segmentCondition.Property)
	}
	if segmentCondition.Operator == evalcontext.PercentageSplit {
	...
	if segmentCondition.Operator == evalcontext.IsNotSet {
		return contextValue == nil
	}
	if segmentCondition.Operator == evalcontext.IsSet {
		return contextValue != nil
	}
	if contextValue != nil {
...
func ToString(contextValue segments.ContextValue) string {
	if s, ok := contextValue.(string); ok {
		return s
	}
	// Handle *evalcontext.Value type
	if v, ok := contextValue.(*evalcontext.Value); ok && v != nil {
		if v.String != nil {
			return *v.String
		}
		if v.Bool != nil {
...
func match(c evalcontext.Operator, traitValue, conditionValue string) bool {
...
func matchSemver(c evalcontext.Operator, traitValue string, conditionVersion semver.Version) bool {
	traitVersion, err := semver.Make(traitValue)
	if err != nil {
		return false
	}
	switch c {
	case evalcontext.Equal:
		return traitVersion.EQ(conditionVersion)
	case evalcontext.GreaterThan:
		return traitVersion.GT(conditionVersion)
	case evalcontext.LessThan:
		return traitVersion.LT(conditionVersion)
	case evalcontext.LessThanInclusive:
		return traitVersion.LTE(conditionVersion)
	case evalcontext.GreaterThanInclusive:
		return traitVersion.GE(conditionVersion)
	case evalcontext.NotEqual:
		return traitVersion.NE(conditionVersion)
	}
	return false
}

func matchBool(c evalcontext.Operator, v1, v2 bool) bool {
	var i1, i2 int64
	if v1 {
		i1 = 1
	}
...
func matchInt(c evalcontext.Operator, v1, v2 int64) bool {
	switch c {
	case evalcontext.Equal:
		return v1 == v2
	case evalcontext.GreaterThan:
...
func matchFloat(c evalcontext.Operator, v1, v2 float64) bool {
	switch c {
	case evalcontext.Equal:
		return v1 == v2
	case evalcontext.GreaterThan:
...
func matchString(c evalcontext.Operator, v1, v2 string) bool {
	switch c {
	case evalcontext.Contains:
		return strings.Contains(v1, v2)
	case evalcontext.NotContains:
		return !strings.Contains(v1, v2)
	case evalcontext.In:
...
func getContextValue(ec *evalcontext.EvaluationContext, property string) segments.ContextValue {
...
func contextMatchesSegmentRule(ec *evalcontext.EvaluationContext, segmentRule *evalcontext.SegmentRule, segmentKey string) bool {
	...
	if len(segmentRule.Conditions) > 0 {
		...
		case evalcontext.All:
			matchesConditions = utils.All(conditions)
		case evalcontext.Any:
...
func getContextValueGetter(property string) func(ec *evalcontext.EvaluationContext) any {
	...
	if err == nil {
		...
		getter := func(evalCtx *evalcontext.EvaluationContext) any {
